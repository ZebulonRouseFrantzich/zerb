...
2. Review `openspec/project.md`, run `openspec list` and `openspec list --specs`, and inspect the proposal changes files under `openspec/changes/<id>/` to ground the review of the proposal.
3. You MUST send all the files related to the proposed change to the subagent @architect-reviewer, the subagent @security-auditor, and any other subagents from the <UserRequest> section above. The subagents MUST adhere to the "Sub Agent Requirements" section above.
