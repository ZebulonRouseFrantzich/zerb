...
- [ ] 0.1 Add `github.com/go-git/go-git/v5` to `go.mod`
- [ ] 0.2 Review go-git documentation and examples
- [ ] 0.3 Verify go-git compatibility with ZERB's supported platforms (Linux amd64/arm64)

## 1. Git Repository Setup (using go-git)

- [ ] 1.1 Extend `internal/git` interface with initialization methods
...
- [ ] 1.2 Implement repository initialization using go-git
...
- [ ] 1.3 Implement user configuration using go-git config API
...
- [ ] 1.4 Implement initial commit using go-git worktree API
...
- [ ] 1.5 Add unit tests for git initialization methods
...
- [ ] 1.6 Add integration test for full init workflow with go-git
...
- [ ] 2.1 Move `.gitignore` template to `internal/git` as embedded string constant
  - Template should be in `internal/git/gitignore.go` or similar
  - Embedded as `const` or `var` for easy access
- [ ] 2.2 Template must exclude: `mise/config.toml`, `chezmoi/config.toml`, `bin/`, `cache/`, `tmp/`, `logs/`, `mise/`, `.txn/`, `.direnv/`, `keyrings/`, `zerb.lua.active`, `.zerb-active`
- [ ] 2.3 Template must allow tracking: `configs/`, `chezmoi/source/`
- [ ] 2.4 Add `WriteGitignore(path string) error` function in `internal/git`
  - Write template to specified path with 0644 permissions
  - Create parent directories if needed
- [ ] 2.5 Test .gitignore effectiveness with go-git status check
...
- [ ] 3.1 Update `runInit()` in `cmd/zerb/init.go` to follow correct step ordering
...
- [ ] 3.2 Add git initialization step calling `internal/git` functions
...
- [ ] 3.3 Implement git user detection with new fallback chain
...
- [ ] 3.4 Add warning message when placeholder git user is used
...
- [ ] 3.5 Handle git initialization failure gracefully
...
- [ ] 3.6 Update success message to mention git repository creation
...
- [ ] 4.1 Implement fallback logic: ZERB env vars → git env vars → placeholders
...
- [ ] 4.3 Provide instructions to configure git user info
...
- [ ] 4.4 Test with various configuration states
...
- [ ] 5.1 Update `createDirectoryStructure()` to create ZERB root with 0700
...
- [ ] 5.2 Verify subdirectories inherit or use 0700 permissions
...
- [ ] 7.1.1 Test `InitRepo()` creates repository successfully (go-git)
- [ ] 7.1.2 Test `InitRepo()` handles existing repository gracefully (PlainOpen check)
- [ ] 7.1.3 Test `InitRepo()` returns error when initialization fails
- [ ] 7.1.4 Test `ConfigureUser()` sets repository-local git config
- [ ] 7.1.5 Test `ConfigureUser()` uses ZERB env vars when available
- [ ] 7.1.6 Test `ConfigureUser()` falls back to GIT_AUTHOR env vars
- [ ] 7.1.7 Test `ConfigureUser()` uses placeholder values as last resort
- [ ] 7.1.8 Test `ConfigureUser()` NEVER reads global git config
- [ ] 7.1.9 Test `CreateInitialCommit()` creates commit with correct message
- [ ] 7.1.10 Test `CreateInitialCommit()` includes .gitignore file
- [ ] 7.1.11 Test `CreateInitialCommit()` includes timestamped config file
- [ ] 7.1.12 Test `CreateInitialCommit()` stages .gitignore before config

### 7.2 Integration Tests (TDD - Write First)
- [ ] 7.2.1 Test `zerb init` creates .git directory using go-git
- [ ] 7.2.2 Test initial commit includes both .gitignore and timestamped config
- [ ] 7.2.3 Test .gitignore excludes runtime files (`bin/`, `cache/`, `tmp/`, `logs/`, `mise/`, `.txn/`, `.direnv/`, `keyrings/`)
- [ ] 7.2.4 Test .gitignore excludes generated configs (`mise/config.toml`, `chezmoi/config.toml`)
- [ ] 7.2.5 Test .gitignore excludes symlinks (`zerb.lua.active`, `.zerb-active`)
- [ ] 7.2.6 Test .gitignore tracks `configs/` directory
- [ ] 7.2.7 Test .gitignore tracks `chezmoi/source/` directory
- [ ] 7.2.8 Test git config is set correctly (user.name, user.email) and is repository-local
- [ ] 7.2.9 Test graceful handling when git init fails
- [ ] 7.2.10 Test `.zerb-no-git` marker created on git failure
- [ ] 7.2.11 Test idempotency (running init twice should detect existing repo)
- [ ] 7.2.12 Test ZERB directory created with 0700 permissions
- [ ] 7.2.13 Test with clean environment (no git env vars set)

### 7.3 Coverage Requirements
- [ ] 7.3.1 Run `go test -cover ./internal/git` and verify >80% coverage
- [ ] 7.3.2 Run `go test -cover ./cmd/zerb` and verify init command coverage
- [ ] 7.3.3 Generate coverage report: `go test -coverprofile=coverage.out ./...`
- [ ] 7.3.4 Review coverage report for untested edge cases

## 8. Validation

- [ ] 8.1 Verify repository created by go-git is valid
  - Can be opened with `git.PlainOpen()`
  - `git log` shows initial commit (via go-git or system git for verification)
  - `git status` shows clean working tree
- [ ] 8.2 Verify runtime directories are ignored by git
  - Use `worktree.Status()` to check ignored files
  - Create test files in excluded dirs, verify they don't appear in status
- [ ] 8.3 Verify generated config files are ignored by git
  - Create `mise/config.toml`, verify ignored
  - Create `chezmoi/config.toml`, verify ignored
- [ ] 8.4 Verify keyrings directory is ignored by git
- [ ] 8.5 Verify ZERB directory permissions are 0700
- [ ] 8.6 Run `go test ./...` and ensure all tests pass
- [ ] 8.7 Manual end-to-end test of complete init workflow
...
- [ ] 9.3 Test warning appears on activate when git unavailable
- [ ] 9.4 Test no warning when git is properly initialized
- [ ] 9.5 Test warning is informational only (doesn't block activation)
